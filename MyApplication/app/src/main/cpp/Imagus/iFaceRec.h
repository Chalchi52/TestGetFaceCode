...
formatted as "Major.Minor.Patch.Build Description string"
For example
    "5.2.3.115 Cuda Compute 3"
@return Verison as a string.

@note This string is only valid until the next API method called on the same thread.
@ingroup err
*/

IFACEAPI const char * ifr_GetVersion(void);
...
/*! @todo {}*/
IFACEAPI IFR_RETURN ifr_LicenceSetCompletionCallback(IFR_CompletionRoutine callback, IFR_CALLBACK_HANDLE cbHandle);
...
/** detemines if the current db has data that needs to be exported to a master db.
This will always return false on a master db, but it is safe to call.
@handleparam{IFR_CONTEXT_HANDLE, context}
@callbackparam{IFR_CompletionRoutine, callback}
@userparam
@return #IFR_TRUE if the database is a master, or #IFR_FALSE if it is a slave, #IFR_DEFERED if the call was deffered
@ingroup facedb
*/
IFACEAPI IFR_RETURN ifr_HasSlaveData(IFR_CONTEXT_HANDLE context, IFR_CompletionRoutine callback DEFAULTPARAM, void *user DEFAULTPARAM);
...
/**
  Experimental WIP and only used for GLES detector
Create a new Image from existing OpenGL texture

This function creates an image handle suitable for use in the library from a texture.
This is useful if you already have the image as a texture.

@note the image memory on the GPU supplied by textureID is not copied, and is required to stay available unmodified
          between the call to the function and the data removal
...
@param time         The @ref time the image was created.
@param creationdata user defined string reperesenting data which is constant throughout all image manipulations.( including crop and rotate)
@param readfunc     user defined function to read the pixels from the GPU texture and crop a region to a CPU buffer

@rethandle{IFR_IMAGE_HANDLE}
@ingroup images
*/
IFACEAPI IFR_IMAGE_HANDLE ifr_CreateImageFromTexture(unsigned int textureID,
                                                     void* context,
                                                     size_t width,
                                                     size_t height,
                                                     IFR_TIME time,
                                                     const char *creationdata,
                                                     void(*readfunc)(unsigned int textureID, void* ctx, int x, int y, int width, int height, void* buf));
...
/**
Creates a rotated image with the given rotation angle

@handleparam{IFR_IMAGE_HANDLE,imageHandle}
@param  angle               The rotation angle to apply to the image.
@rethandle{IFR_IMAGE_HANDLE}
@ingroup images

*/
IFACEAPI IFR_IMAGE_HANDLE ifr_ImageGetRotatedImage(IFR_IMAGE_HANDLE imageHandle, float angle);
...
@param size                 The width of the cropping square
@rethandle{IFR_IMAGE_HANDLE}
@ingroup images
...
/** 
Returns the blurriness measurement of a face object.
The greater the value, the sharper the face image.
**/
IFACEAPI float ifr_FaceGetBlurriness(IFR_FACE_HANDLE FaceHandle);
...
@handleparam{IFR_PERSON_HANDLE,person}
@return A C string containing a dictionary of key-value pairs in JSON format. @objectlocal
@ingroup person
*/
IFACEAPI const char* ifr_PersonGetDetails(IFR_PERSON_HANDLE person);

/**
Set the details of a person.
@handleparam{IFR_PERSON_HANDLE,person}
@param details A C string containing a dictionary of key-value pairs in JSON format. @objectlocal
@ingroup person
*/
IFACEAPI IFR_RETURN ifr_PersonSetDetails(IFR_PERSON_HANDLE person, const char* details);
...
@ingroup facedb
*/
IFACEAPI IFR_BUFFER_HANDLE ifr_ExportPersonToBuffer2(IFR_CONTEXT_HANDLE contextHandle, IFR_UUID personID, IFR_BufferCompletionRoutine callback DEFAULTPARAM, void* user DEFAULTPARAM);
...
@ingroup facedb
*/
IFACEAPI IFR_BUFFER_HANDLE ifr_ExportSyncInfoToBuffer(IFR_CONTEXT_HANDLE contextHandle, IFR_BufferCompletionRoutine callback DEFAULTPARAM, void* user DEFAULTPARAM);


/** Gets all information from a slave/master to complete syncronisation to its master/slave
the process is started by reqesting from either the master or the slave and passing the result to the other db, this ping pongs until a null result is given indicating it is complete. 
using a nullptr entry to the master will get the entire DB as the slave is unknown.
@handleparam{IFR_CONTEXT_HANDLE,contextHandle}
@callbackparam{IFR_HandleCompletionRoutine,callback}
@userparam
@rethandle{IFR_BUFFER_HANDLE}
@ingroup facedb
*/
IFACEAPI IFR_BUFFER_HANDLE ifr_PerformDBSync(IFR_CONTEXT_HANDLE contextHandle, IFR_BUFFER_HANDLE bufferHandle, IFR_BufferCompletionRoutine callback DEFAULTPARAM, void* user DEFAULTPARAM);
...
@ingroup facedb
*/
IFACEAPI IFR_INTS_HANDLE ifr_GetDbFeatures(IFR_CONTEXT_HANDLE context, IFR_IntsCompletionRoutine callback DEFAULTPARAM, void *user DEFAULTPARAM);
...
@callbackparam{IFR_UUIDCompletionRoutine,callback}
@userparam
@return if the callback was not supplied, the audit identifer, otherwise NullUUID, and the result comes in the callback. @setslasterror
@ingroup facedb
*/
IFACEAPI IFR_UUID ifr_GetLastAudit(IFR_CONTEXT_HANDLE context, IFR_UUIDCompletionRoutine callback DEFAULTPARAM, void *user DEFAULTPARAM);
...
@handleparam{IFR_DETECTOR_HANDLE,detector}
@handleparam{IFR_IMAGE_HANDLE,image}
@handleparam{IFR_IMAGE_HANDLE,colourImage} Optional colour image from which to crop faces from
@param flags     A flag denoting which type of detection to perform see #IFR_DETECTION_FLAGS
@callbackparam{IFR_DetectionCompletionRoutine,callback}
@userparam
@deferred_rethandle{IFR_DETECTOR_RESULT_HANDLE}
@ingroup facedetection
*/
IFACEAPI IFR_DETECTOR_RESULT_HANDLE ifr_DetectFacesInImage3(IFR_DETECTOR_HANDLE detector, IFR_IMAGE_HANDLE image, IFR_IMAGE_HANDLE colourImage, IFR_DETECTION_FLAGS flags, IFR_DetectionCompletionRoutine callback DEFAULTPARAM, void *user DEFAULTPARAM);
...
/** @getitem{#IFR_IMAGE_HANDLE,IFR_DETECTOR_RESULT_HANDLE,handle}
@sa #ifr_GetNumStrings
@ingroup facedetection
*/
IFACEAPI IFR_IMAGE_HANDLE ifr_DetectorResultGetFaceImage(IFR_DETECTOR_RESULT_HANDLE result, size_t index);
...
/**
Sets the minimum quality for faces to be detected
@handleparam{IFR_DETECTOR_HANDLE,detector}
@param minQuality minimum quality
@ifr_return
@ingroup facedetection
*/
IFACEAPI IFR_RETURN ifr_DetectorSetMinFaceQuality(IFR_DETECTOR_HANDLE detector, float minQuality);

/**
Sets the minimum blurriness for faces to be detected
@handleparam{IFR_DETECTOR_HANDLE,detector}
@param minBlurriness minimum blurriness
@ifr_return
@ingroup facedetection
*/
IFACEAPI IFR_RETURN ifr_DetectorSetMinFaceBlurriness(IFR_DETECTOR_HANDLE detector, float minBlurriness);

/**
Sets the minimum faceness for faces to be detected
@handleparam{IFR_DETECTOR_HANDLE,detector}
@param minFaceness minimum faceness
@ifr_return
@ingroup facedetection
*/
IFACEAPI IFR_RETURN ifr_DetectorSetMinFaceFaceness(IFR_DETECTOR_HANDLE detector, float minFaceness);
...
@deprecated use ifr_GetConfidence2 instead

**/
IFACEAPI float ifr_GetConfidence(float dist);

/**
Returns a smooth confidence from DNN face distance
@deprecated use ifr_GetConfidence2 instead
**/
...
/**
Get face confidence from DNN face distance.
@param dist Distance returned from a DNN algorithm
@param algo The algorithm that returned this distance
@return A percentage confidence from 0 to 100
**/
IFACEAPI float ifr_GetConfidence2(float dist, IFR_ALGORITHM algo);

//! @brief Utility function to convert flags to angle
IFACEAPI int ifr_angleFromDetectionFlags(IFR_DETECTION_FLAGS flags);

//! @brief Utility function to convert angle to flags
IFACEAPI IFR_DETECTION_FLAGS ifr_detectionFlagsFromAngle(int angle);
...
inline std::ostream & operator << (std::ostream & os, IFR_UUID const & uid)
{
    os << ifr_UUIDToChar2(uid);
    return os;
}
