...
@{ */
typedef int32_t IFR_SEARCH_FLAGS;
static const IFR_SEARCH_FLAGS IFR_SEARCH_NONE = 0;          ///< No special flags
...
static const IFR_VERIFY_FLAGS IFR_VERIFY_V3 = 3;   ///< use IFRV3 feature for verification (DEFAULT)
/*! @} */
...
@{ */
typedef int32_t IFR_TAG_FLAGS;
static const IFR_TAG_FLAGS IFR_TAG_NONE = 0; ///< No flags
...
typedef enum IFR_IMAGE_FORMAT {
    ...
    IFR_NV12 = 7,           ///< YYYY UUVV (CUDA)
    IFR_NV21 = 8,           ///< YYYY VVUU (Android)
} IFR_IMAGE_FORMAT;
...
static const IFR_IMAGE_COMPRESSION IFR_IMAGE_COMPRESSION_WEBP = (1 << 3);    ///< Use Webp compression
...
typedef enum IFR_ALGORITHM {
    ...
    IFR_ALGORITHM_DEFAULT_V3 = 0x20     ///< Version 3 features, use this for verification
} IFR_ALGORITHM;
...
typedef enum IFR_FEATURESET {
    ...
    IFR_FEATURE_V3 = 0x20   ///<  This featureset is ideal for verification
} IFR_FEATURESET;
...
IFR_DETECTION_NONE                  | Dont detect any faces, can be useful as the images using the same detector handle are returned in calling order to control how many frames of a video have the face detector run against it without changing the order
...
IFR_DETECTION_DNN				    | This flag turns on the DNN face detector
IFR_DETECTION_ROTATED90             | Hint that the input image was rotated 90 degrees. Only used for GL Detector
IFR_DETECTION_ROTATED180            | Hint that the input image was rotated 180 degrees. Only used for GL Detector
IFR_DETECTION_ROTATED270            | Hint that the input image was rotated 270 degrees. Only used for GL Detector
IFR_DETECTION_ROTATED               | Mask for hint that the input image was rotated by some angle
...
static const IFR_DETECTION_FLAGS IFR_DETECTION_DNN = (1 << 8);
static const IFR_DETECTION_FLAGS IFR_DETECTION_ROTATED90 = (1 << 9);
static const IFR_DETECTION_FLAGS IFR_DETECTION_ROTATED180 = (1 << 10);
static const IFR_DETECTION_FLAGS IFR_DETECTION_ROTATED270 = (1 << 11);
static const IFR_DETECTION_FLAGS IFR_DETECTION_ROTATED = (1 << 9) | (1 << 10) | (1 << 11);
...
typedef struct IFR_DETECTORRESULT
{
	...
	float blurriness;			///< measure the blurriness of the grayscale image.
} IFR_DETECTORRESULT;
...
typedef void(*IFR_IntsCompletionRoutine)(IFR_RETURN, IFR_INTS_HANDLE, void *);                                                   ///< Callback function for when an #IFR_INTS_HANDLE is returned. 
typedef void (*IFR_BufferCompletionRoutine)(IFR_RETURN, IFR_BUFFER_HANDLE, void *);                                                   ///< Callback function for when an #IFR_BUFFER_HANDLE is returned. 
